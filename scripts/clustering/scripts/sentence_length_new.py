...
parser.add_argument('--keep-length-part', action='store_true')
...
if args.keep_length_part:
    print("Keeping only first ", args.length, " words of each line")
else:
...
with open(args.text_file) as f:
    for line in f.readlines():
        if len((line.strip()).split()) < args.length:
            ...
            if args.keep_length_part:
                out.write(" ".join((line.strip()).split()[:args.length]) + "\n")
            else:
